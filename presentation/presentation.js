...
async function exportToPPTX() {
    ...
    try {
        ...
        // ==================== Slide 11: Journey Overview ====================
        slide = pptx.addSlide({ masterName: 'GALAXY_MASTER' });
        slide.addText('🚀 Our Development Journey', { x: 0.5, y: 0.3, w: 9, h: 0.6, fontSize: 28, bold: true, color: colors.cyan });
        slide.addText('How we built Galaxy Weather using Kanban methodology', { x: 0.5, y: 1, w: 9, h: 0.4, fontSize: 14, color: colors.textMuted, align: 'center' });
        
        const journeyStats = [
            { num: '5', label: 'Weeks' },
            { num: '24', label: 'Tasks Completed' },
            { num: '7', label: 'UML Diagrams' },
            { num: '100%', label: 'Requirements Met' }
        ];
        
        journeyStats.forEach((stat, idx) => {
            const x = 0.5 + (idx * 2.4);
            slide.addShape(pptx.shapes.RECTANGLE, { x: x, y: 2, w: 2.2, h: 1.5, fill: '1a1a3a', line: { color: colors.cyan, pt: 1 } });
            slide.addText(stat.num, { x: x, y: 2.2, w: 2.2, h: 0.7, fontSize: 32, bold: true, color: colors.cyan, align: 'center' });
            slide.addText(stat.label, { x: x, y: 2.9, w: 2.2, h: 0.4, fontSize: 12, color: colors.textMuted, align: 'center' });
        });

        // ==================== Slides 12-16: Trello Journey ====================
        const trelloSlides = [
            { week: 'Week 1', title: 'Project Kickoff', desc: 'Setting up Trello board and defining backlog', imgs: ['1.png', '2.png'] },
            { week: 'Week 2-3', title: 'Development Phase', desc: 'Backend and Frontend development in parallel', imgs: ['3.png', '4.png', '5.png'] },
            { week: 'Week 4', title: 'Testing & Documentation', desc: 'UML diagrams and feature testing', imgs: ['6.png', '7.png'] },
            { week: 'Week 5', title: 'Project Completion', desc: 'Final review and deployment', imgs: ['8.png', '9.png'] }
        ];

        for (const t of trelloSlides) {
            slide = pptx.addSlide({ masterName: 'GALAXY_MASTER' });
            slide.addText(`📋 ${t.week}: ${t.title}`, { x: 0.5, y: 0.3, w: 9, h: 0.6, fontSize: 28, bold: true, color: colors.cyan });
            slide.addText(t.desc, { x: 0.5, y: 0.9, w: 9, h: 0.4, fontSize: 14, color: colors.textMuted, align: 'center' });
            
            // Add image placeholders (note: actual Trello screenshots would need to be embedded)
            const imgWidth = t.imgs.length === 3 ? 2.8 : 4;
            const startX = t.imgs.length === 3 ? 0.7 : 1;
            const gap = t.imgs.length === 3 ? 0.3 : 0.5;
            
            t.imgs.forEach((img, idx) => {
                const x = startX + (idx * (imgWidth + gap));
                slide.addShape(pptx.shapes.RECTANGLE, { 
                    x: x, y: 1.5, w: imgWidth, h: 2.5, 
                    fill: '1a1a3a', line: { color: '444466', pt: 1 }
                });
                slide.addText(`📸\n[${img}]\n\nTrello Screenshot`, { 
                    x: x, y: 2.2, w: imgWidth, h: 1.5, 
                    fontSize: 12, color: colors.textMuted, align: 'center', valign: 'middle'
                });
            });
        }
        ...
        const nfrs = [
            { icon: '⚡', title: 'Performance', items: ['Response < 2s', 'Load < 3s', 'API < 500ms'] },
            { icon: '🛡️', title: 'Reliability', items: ['99.9% uptime', 'Auto failover', 'Error handling'] },
            { icon: '🔒', title: 'Security', items: ['HTTPS', 'API protection', 'Input validation'] },
            { icon: '📈', title: 'Scalability', items: ['Horizontal scale', 'Cloud ready', 'Microservices'] }
        ];
        
        nfrs.forEach((nfr, idx) => {
            const x = 0.5 + (idx * 2.4);
            slide.addShape(pptx.shapes.RECTANGLE, { x: x, y: 1.1, w: 2.2, h: 2.5, fill: '1a1a3a', line: { color: '444466', pt: 1 } });
            slide.addText(`${nfr.icon} ${nfr.title}`, { x: x, y: 1.2, w: 2.2, h: 0.4, fontSize: 12, bold: true, color: colors.cyan, align: 'center' });
            nfr.items.forEach((item, i) => {
                slide.addText(`• ${item}`, { x: x + 0.2, y: 1.7 + (i * 0.4), w: 1.8, h: 0.35, fontSize: 10, color: colors.text });
            });
        });

        // ==================== Slides 18-24: UML Diagrams ====================
        const umlSlides = [
            { title: 'Use Case Diagram', file: '../files/01_use_case_diagram.svg', desc: 'System functionality from user perspective' },
            { title: 'Class Diagram', file: '../files/02_class_diagram.svg', desc: 'System structure and class relationships' },
            { title: 'Sequence Diagram', file: '../files/03_sequence_diagram.svg', desc: 'Interaction flow for forecast request' },
            { title: 'Activity Diagram', file: '../files/04_activity_diagram.svg', desc: 'Workflow process for forecast generation' },
            { title: 'State Diagram', file: '../files/05_state_diagram.svg', desc: 'Forecast request lifecycle states' },
            { title: 'Component Diagram', file: '../files/06_component_diagram.svg', desc: 'System components and their interactions' },
            { title: 'ER Diagram', file: '../files/07_er_diagram.svg', desc: 'Database entity relationships' }
        ];
        ...
        for (const uml of umlSlides) {
            ...
            // Try to add actual image, fallback to placeholder
            try {
                slide.addImage({ 
                    path: uml.file,
                    x: 1, y: 1.5, w: 8, h: 3.2,
                ...
                slide.addText(`📐\n${uml.title}\n\n[See presentation for full diagram]`, { 
                    x: 1, y: 2.2, w: 8, h: 2, 
        ...
        // ==================== Slide 25: Hybrid Approach ====================
        slide = pptx.addSlide({ masterName: 'GALAXY_MASTER' });
        slide.addText('🔀 Hybrid Forecasting Approach', { x: 0.5, y: 0.3, w: 9, h: 0.6, fontSize: 28, bold: true, color: colors.cyan });
        
        // API Only
        slide.addShape(pptx.shapes.RECTANGLE, { x: 0.5, y: 1.2, w: 4, h: 2.2, fill: '1a1a3a', line: { color: '444466', pt: 1 } });
        slide.addText('API Only Approach', { x: 0.5, y: 1.3, w: 4, h: 0.4, fontSize: 14, bold: true, color: colors.text, align: 'center' });
        slide.addText('✓ Fast\n✓ Real-time\n❌ No patterns\n❌ Less accurate', { x: 0.7, y: 1.8, w: 3.6, h: 1.3, fontSize: 12, color: colors.textMuted });
        
        // Our Hybrid
        slide.addShape(pptx.shapes.RECTANGLE, { x: 5, y: 1.2, w: 4.5, h: 2.2, fill: '2d1b4e', line: { color: colors.cyan, pt: 2 } });
        slide.addText('Our Hybrid Approach ⭐', { x: 5, y: 1.3, w: 4.5, h: 0.4, fontSize: 14, bold: true, color: colors.cyan, align: 'center' });
        slide.addText('✓ API baseline\n✓ Trend analysis\n✓ Pattern recognition\n✓ Higher accuracy', { x: 5.2, y: 1.8, w: 4.1, h: 1.3, fontSize: 12, color: colors.text });
        ...
        const futureItems = [
            { icon: '🤖', text: 'Machine Learning Integration' },
            { icon: '🔔', text: 'Weather Alerts & Push Notifications' },
            { icon: '🗺️', text: 'Interactive Weather Maps' },
            { icon: '📱', text: 'Native Mobile Application' },
            { icon: '📊', text: 'Advanced Historical Visualization' },
            { icon: '👥', text: 'User Accounts & Personalization' }
        ];
        
        futureItems.forEach((item, idx) => {
            const x = (idx % 2) * 4.7 + 0.5;
            const y = Math.floor(idx / 2) * 0.8 + 1.1;
            slide.addShape(pptx.shapes.RECTANGLE, { x: x, y: y, w: 4.5, h: 0.7, fill: '1a1a3a', line: { color: '444466', pt: 1 } });
            slide.addText(`${item.icon} ${item.text}`, { x: x, y: y, w: 4.5, h: 0.7, fontSize: 13, color: colors.text, align: 'center', valign: 'middle' });
        });
        ...
        slide.addText('Kish International Campus • Sharif University', { x: 0.5, y: 4.3, w: 9, h: 0.3, fontSize: 12, color: colors.textMuted, align: 'center' });
        slide.addText('❓ Questions?', { x: 0.5, y: 4.7, w: 9, h: 0.5, fontSize: 18, color: colors.cyan, align: 'center' });
        ...
        alert('Export error: ' + error.message + '\n\nNote: UML diagram images cannot be embedded from SVG files. The slides contain placeholders instead.');
